...
bool Game::CreateObjectsFromFile(const std::string& fileName)
{
    std::fstream fs{ fileName + ".obl",std::ios_base::in | std::ios_base::binary };
    size_t size;
    fs >> size;
    for (int i = 0; i < size; ++i)
    {
        ObjectBuffer buffer;
        std::getline(fs, buffer._type);
        std::getline(fs, buffer._objectName);
        std::getline(fs, buffer._fileName);
        std::getline(fs, buffer._text);
        fs >> buffer._originPos.x >> buffer._originPos.y;
        fs >> buffer._velocity.x >> buffer._velocity.y;
        fs >> buffer.hasTarget;
        fs >> buffer._targetPos.x >> buffer._targetPos.y;
        fs >> buffer.isRect;
        fs >> buffer.widthHeight.x >> buffer.widthHeight.y;
        fs >> buffer.rectColor.r >> buffer.rectColor.g >> buffer.rectColor.b >> buffer.rectColor.a;
        fs.ignore();
        std::string mType;
        std::getline(fs, mType);
        if (mType == "default")
            buffer._mType = MoveType::DEFAULT;
        else if (mType == "square")
            buffer._mType = MoveType::SQUARE;

        SDL_Rect bufferRect;
        bufferRect.x = 0; bufferRect.y = 0;
        bufferRect.w = buffer.widthHeight.x;
        bufferRect.h = buffer.widthHeight.y;
        SDL_Color bufferColor;
        bufferColor.r = buffer.rectColor.r; bufferColor.g = buffer.rectColor.g;
        bufferColor.b = buffer.rectColor.b; bufferColor.a = buffer.rectColor.a;
        if (buffer._type == "GameObject")
        {
            if (buffer.isRect)
            {
                if (buffer.hasTarget)
                {
                    CreateMoveTargetRect<GameObject>(buffer._objectName, bufferRect,
                        new Vector2F[3]{ buffer._originPos,buffer._velocity,buffer._targetPos },
                        bufferColor,buffer._mType);
                }
                else
                {
                    CreateRect<GameObject>(buffer._objectName, bufferRect,
                        new Vector2F[2]{ buffer._originPos,buffer._velocity },
                        bufferColor, buffer._mType);
                }
            }
            else
            {
                if (buffer.hasTarget)
                {
                    CreateMoveTargetObject<GameObject>(buffer._objectName, buffer._fileName,
                        new Vector2F[3]{ buffer._originPos,buffer._velocity,buffer._targetPos }, buffer._mType);
                }
                else
                {
                    CreateObject<GameObject>(buffer._objectName, buffer._fileName,
                        new Vector2F[2]{ buffer._originPos,buffer._velocity }, buffer._mType);
                }
            }
        }
        else if (buffer._type == "Button")
        {
            if (buffer.hasTarget)
            {
                CreateMoveTargetObject<Button>(buffer._objectName, buffer._fileName,
                    new Vector2F[3]{ buffer._originPos,buffer._velocity,buffer._targetPos }, buffer._mType);
            }
            else
            {
                CreateObject<Button>(buffer._objectName, buffer._fileName,
                    new Vector2F[2]{ buffer._originPos,buffer._velocity }, buffer._mType);
            }
        }
        else if (buffer._type == "TextButton")
        {
            CreateTextObject<TextButton>(buffer._objectName, buffer._fileName,
                buffer._text, new Vector2F[2]{ buffer._originPos,buffer._velocity }, buffer._mType);
        }
